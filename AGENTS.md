...
- 恢复包可以包含运行配置中的 rclone.conf，因此恢复包必须作为敏感文件保存。
...
导入 `formatVersion=2` 恢复包时应尽量恢复到导出时状态：同名项覆盖，恢复包中不存在的仓库、通知、默认变量和 rclone.conf 可被删除。
